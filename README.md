...
| `@lyx-apply` | **当前会话本人实施**：读 tasks.md 逐任务实施 + 验证 + 勾 checkbox → commit `apply: <change>`（无外部委托、无 routing/implementer 概念） |
| `@lyx-archive` | 归档完成的 change（委托 `@openspec-archive-change skill`）+ 自动 commit |
| `@lyx-review-plan` | 审方案：`codex exec` 独立子会话（角色词 `plan-reviewer.md`）分级审查，审查-修复循环直到 Critical 清零或触发终止条件（全局轮数上限 5，清零优先），清零统一提交修复 |
...
@lyx-review-plan            # 审方案（codex exec 子会话）
@lyx-apply                  # 本会话实施 + 立即 commit
@lyx-review-code            # 审代码（codex exec 子会话）
@lyx-archive
```
