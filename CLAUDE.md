...
安装产物：`~/.codex/prompts/ly-*.md`（14 个 `/ly:*` 命令）+ `~/.ly/prompts/codex/`（8 个角色提示词）+ `~/.ly/config.toml`（配置，自动从旧 `~/.claude/.ly/config.toml` 迁移）。

## 14 个 /ly:* 命令

| 命令 | 一句话说明 |
|------|-----------|
| `/ly:init` | 生成项目 AGENTS.md + `openspec init` + 自动 commit |
| `/ly:explore` | 委托 `opsx:explore`（纯薄壳） |
| `/ly:propose` | 编排入口：隔离三选一 → 全自动/手动 → `opsx:propose` → 方案自审 → commit `propose:`；全自动 = review-plan → apply → review-code 流水线 |
| `/ly:apply` | 当前会话本人读 tasks.md 逐任务实施 + 验证 + 勾 checkbox → commit `apply:` |
| `/ly:archive` | 委托 `opsx:archive` + 自动 commit |
| `/ly:review-plan` | `codex exec` 独立子会话审方案（`plan-reviewer.md`），审查-修复循环，清零统一提交 |
| `/ly:review-code` | `codex exec` 独立子会话审代码（`reviewer.md`），Critical/Warning/Info 分级，同循环 |
| `/ly:release` | GitFlow 四场景发版 + SemVer 推导 + 上线合并二选一 |
| `/ly:changelog` | Keep a Changelog 格式生成/更新 CHANGELOG.md |
| `/ly:publish` | npm 包发布四场景 |
| `/ly:commit` `/ly:rollback` `/ly:clean-branches` `/ly:worktree` | Git 工具（worktree 无 `switch`） |
