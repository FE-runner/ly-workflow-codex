> **DEPRECATED（不适用于本项目）**：本能力描述的是对 claude 侧上游遗产的清理，已不适用；现状为 codex 单宿主下 `legacy-cleanup.ts` 只清理 `~/.codex/` 侧的 LY 残留（AGENTS.md 管理区块、config.toml 旧注释与 `[features.multi_agent_v2]`、`agents/ly-*.toml`）。本能力不再适用，内容仅作历史参考。
