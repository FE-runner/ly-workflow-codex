...
## Purpose
定义 codex 单宿主（subagent 多 Agent 模式）下子代理模型指定与校验契约：三个模型字段（reviewModel / reviewModelB / codingModel）经"模板指示 + 宿主能力"落实，spawnableModels 作为模型候选与校验的唯一清单来源，lycx doctor 与 CLI 向导据此校验模型配置。
