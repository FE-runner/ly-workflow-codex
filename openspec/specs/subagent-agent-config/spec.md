...
## Purpose
定义 codex 单宿主（subagent 多 Agent 模式）下子代理模型指定与校验契约：三个模型字段（reviewModel / reviewModelB / codingModel）经"模板指示 + 宿主能力"落实，spawnableModels 作为模型候选与校验的基准清单来源（已配置模型字段值始终并入生效清单、codex 当前主模型在未显式配置清单时并入，用户已配置的模型按配置列出，并保留自定义输入入口），lycx doctor 与 CLI 向导据此校验模型配置。
...
`~/.ly/config.toml` 的 `[codexHost]` 节 SHALL 提供可选的 `spawnableModels` 字符串数组字段：声明当前宿主显式 spawn 子代理可用的模型清单。该字段未配置、空白或清洗后为空数组时，SHALL 回退使用内置默认常量 `SPAWNABLE_MODELS_DEFAULT`（=`gpt-6-astra`、`gpt-5.6-sol`、`gpt-5.6-terra`、`gpt-5.6-luna`、`gpt-5.5`）。清洗规则 SHALL 为：仅保留非空字符串、逐项 trim、去重；清洗后空数组等价未配置。字段显式存在但整体格式非法（如非数组字面量、无法解析为字符串数组的形态）时，SHALL 按未配置回退内置默认，并在 `lycx doctor` 检查项中对该形态输出 WARN 提示（区别于完全未配置的静默通过）。`spawnableModels` SHALL 作为模型字段候选与校验的**基准**清单来源；**生效清单**解析规则 SHALL 为：基准 = `spawnableModels` 清洗后合法非空清单（未配置 / 格式非法 / 清洗后为空时回退内置默认 `SPAWNABLE_MODELS_DEFAULT`）；`[codexHost]` 已配置的 `reviewModel`/`reviewModelB`/`codingModel` 非空值（含向导自定义输入）SHALL **始终并入**生效清单——用户显式指定的模型（无论来自清单选择还是自定义输入）按配置列出，不被基准清单误判；codex 当前主模型（`~/.codex/config.toml` 顶层 `model`，可检测时）仅在未显式配置 `spawnableModels`（宽松路径）时并入，用户显式维护清单（严格路径）时不并入，留空字段继承主模型由 doctor 交叉校验 WARN 提示。既有 `reviewModel`/`reviewModelB`/`codingModel` 的非空取值 SHALL 落在此生效清单内才视为合法；SHALL NOT 以 provider `/models` 拉取结果或 `~/.codex/models.json` 注册集合作为候选/校验来源（实测二者均不等于 spawn 可用列表）。

#### Scenario: 未配置 spawnableModels 时使用内置默认（宽松路径）
- **WHEN** 用户 `~/.ly/config.toml` 的 `[codexHost]` 未配置 `spawnableModels`
- **THEN** 模型三连候选与 doctor 校验使用生效清单（宽松路径）= 内置默认五模型（gpt-6-astra / gpt-5.6-sol / gpt-5.6-terra / gpt-5.6-luna / gpt-5.5）∪ 已配置模型字段值 ∪ codex 当前主模型（可检测时），流程不受影响

#### Scenario: 未配置 spawnableModels 时按配置的模型列出
- **WHEN** 用户未配置 `spawnableModels`，`~/.codex/config.toml` 顶层 `model = "deepseek-v4.1-flash"`，且/或 `[codexHost]` 已配置 `reviewModel = "glm-5.3-flash"`
- **THEN** 模型三连候选与 doctor 生效清单包含 `deepseek-v4.1-flash` 与 `glm-5.3-flash`（与内置默认去重），二者均判定为合法，SHALL NOT 因不在内置默认而误报"配置无效"

#### Scenario: 用户按实测维护 spawnableModels
- **WHEN** 用户实测本机 spawn 可用列表与内置默认不同（如仅 glm-5.3-flash 可用），将 `spawnableModels = ["glm-5.3-flash"]` 写入配置
- **THEN** `lycx init`/菜单候选与 doctor 校验以该用户清单为基准，模型三连可从该清单选择；已配置的模型字段值仍并入生效清单（按配置列出）
...
模型指定 SHALL 以模板内明确的模型指示落实（模板写明各 subagent 的模型取哪个配置字段、未配置用当前会话模型），由运行环境的宿主 spawn 能力执行；模板 SHALL NOT 依赖任何 shell 层模型参数。**未配置（留空）**时 SHALL 回退为继承当前会话模型，SHALL NOT 阻断安装或审查流程。**非空配置值 ∉ 生效清单**（生效清单 = `spawnableModels` 基准或内置默认；已配置模型字段值始终并入、codex 主模型在宽松路径并入，见 Req2）SHALL 判定为配置无效：明确报告"子代理模型配置无效（<model> 不在可用列表，请运行 `lycx doctor` 或配置 `[codexHost] spawnableModels`）"，停止该关卡转人工改配，SHALL NOT 以笼统的"运行期失败/审查调用失败"终止条件掩盖，SHALL NOT 回退为当前会话直接执行。运行环境无 subagent 能力或初始 spawn 失败（配置合法时）按回退口径回退当前会话直接执行并如实报告"已回退，原因：subagent 不可用"，该回退 SHALL NOT 视为流程失败。spawn 失败报错原文（含 `Available models: ...` 时）SHALL 如实展示，并可提示用户据此维护 `spawnableModels`。模板运行时读取 `~/.ly/config.toml` 失败（缺文件/解析错误）SHALL 视为"配置状态未知"：明确提示"无法读取配置，请运行 `lycx doctor` 检查"，SHALL NOT 按"未配置"静默继承回退。
...
#### Scenario: 显式清单下配置了清单外模型（并入后通过）
- **WHEN** 用户显式配置 `spawnableModels = ["gpt-5.6-luna"]` 且 `[codexHost] reviewModelB = "glm-5.3-flash"` ∉ 该清单，运行 `@lyx-review-code`
- **THEN** 生效清单并入 `glm-5.3-flash`（已配置字段值始终并入），校验通过，审查 agent B 使用该模型 spawn，SHALL NOT 误判"配置无效"

#### Scenario: 未配置 spawnableModels 时配置的模型按配置列出（宽松路径）
- **WHEN** 用户未配置 `spawnableModels` 且 `[codexHost] reviewModel = "glm-5.3-flash"`（不在内置默认），运行 `@lyx-review-plan`
- **THEN** 生效清单（宽松路径）并入 `glm-5.3-flash`，校验通过，审查 agent A 使用该模型 spawn，SHALL NOT 误判"配置无效"
...
`lycx init` 向导 SHALL 在交互模式采集 `codexHost` 的三个模型字段：`reviewModel`（审查 agent A）、`reviewModelB`（审查 agent B）、`codingModel`（coding 实施）。交互流程 SHALL 为"语言 → 选定 API 提供方 → **Codex 现状检测展示** → 模型三连 → 配置摘要"，SHALL NOT 包含"工作流模式"与"选择 Agent"等单选项步骤。**现状检测 SHALL 为纯静态读取**：展示 `~/.codex/config.toml` 顶层 `model`（主会话模型，即"默认继承"的实际值）、`[model_providers.*]` 条目、`~/.codex/models.json` 注册集合规模；读取失败或缺文件时 SHALL 如实标注"未检测到"，SHALL NOT 阻断流程。现状检测展示块 SHALL 附带 `reasoning_effort` 参数坑背景提示（如部分第三方模型默认推理参数不可用、需显式 `low`；该提示为背景说明，SHALL NOT 新增模板参数通道）。**模型三连候选 SHALL = `默认继承当前会话模型（留空）` + 生效清单 + `自定义输入`**。生效清单 = `spawnableModels` 配置值或内置默认，已配置模型字段值始终并入、codex 当前主模型在未显式配置清单时并入（用户已配置的模型按配置列出）。**自由输入入口 SHALL 保留**：候选末项"自定义输入模型…"，选择后输入任意模型名（可不在生效清单内），经 trim 后作为该字段配置值，空白视为取消；逐字段 list 选择；采集 SHALL NOT 以选定 provider 的 `/models` 拉取结果为候选（自由输入不依赖 /models 拉取）。每个字段默认值语义：既有值非空且 ∈ 候选清单 → 默认该项；既有值非空但 ∉ 候选清单 → 候选清单附加"保留当前值 `<value>`（不在可用列表，警告）"项并默认该项，SHALL NOT 静默丢弃或替换；无既有值或空白 → 默认"留空"。交互 init、非交互模式（`--skip-prompt`）与 menu 单字段编辑等所有重写 `[codexHost]` 的路径 SHALL 保留既有 `reviewModelB`/`codingModel` 与 `spawnableModels`，SHALL NOT 因重装或编辑而丢弃。menu 的审查模型编辑 SHALL 使用同一候选清单与默认值语义。
...
- **THEN** 候选 = "留空（默认继承） + 生效清单（配置值或内置默认，已配置字段值始终并入、主模型在未显式配置清单时并入） + 自定义输入"，任意环境（含 /models 不可达）下模型三连均可正常选择，不阻断安装

#### Scenario: Codex 现状检测展示主模型
- **WHEN** `~/.codex/config.toml` 顶层配置 `model = "DeepSeek-V4-Flash-0731"`，用户运行 `lycx init` 进入模型三连前
- **THEN** 现状检测展示主会话模型 `DeepSeek-V4-Flash-0731` 与 provider 信息，"默认继承"选项标注该实际值；未显式配置 `spawnableModels` 时该主模型同时进入模型三连候选（宽松路径并入）
...
`lycx doctor` SHALL 提供"Codex 子代理模型配置"检查项：读取 `codexHost.reviewModel`/`reviewModelB`/`codingModel`，以**生效清单**（= `spawnableModels` 或内置默认；已配置模型字段值始终并入、codex 当前主模型在未显式配置清单时并入，见 Req2）逐一判定——留空 = 通过（回退当前会话模型继承，实测可用）；非空且属于生效清单 = 通过；非空且不属于生效清单 = FAIL（字段值已并入，正常路径不可达，保留为兜底判定），提示"子代理模型配置无效（<model> 不在可用列表），请配置 `[codexHost] spawnableModels` 或改用列表内模型"。检测结果 SHALL 展示所用清单（内置默认或用户配置）。留空 = 通过的前提 SHALL 在判定文案中注明（以主会话模型本身可 spawn 为前提）；`spawnableModels` 字段显式存在但格式非法/清洗后为空时 SHALL 输出 WARN 提示该形态（区别于未配置），模型字段本身仍按生效清单判定。
...
#### Scenario: 显式清单下配置了清单外模型（并入后通过）
- **WHEN** 用户显式配置 `spawnableModels = ["glm-5.3-flash"]` 且 `reviewModel = "deepseek-v4-flash"`（∉ 用户清单），运行 `lycx doctor`
- **THEN** 该字段值并入生效清单，判定通过（用户显式指定的模型按配置列出），不报 FAIL

#### Scenario: 未配置 spawnableModels 时已配置模型与主模型并入清单（宽松路径通过）
- **WHEN** 用户未配置 `spawnableModels`，`~/.codex/config.toml` 顶层 `model = "deepseek-v4.1-flash"` 且 `[codexHost] reviewModel = "glm-5.3-flash"`，运行 `lycx doctor`
- **THEN** 生效清单（宽松路径）= 内置默认 ∪ `deepseek-v4.1-flash` ∪ `glm-5.3-flash`，三个字段判定均通过，输出无 WARN/FAIL
