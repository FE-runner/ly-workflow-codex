...
- **影响**：删除 `init.mode.select/singleAgent/agentSelect/agentCodex` i18n 键；`collectCodexHostConfig` 的 `askAgent` 参数删除。
...
- **默认值语义**（防静默丢值）：既有配置值在列表内 → 默认该项；既有值非空但不在列表 → 默认"自定义输入"；否则（无既有值）→ 默认"不设置"。
