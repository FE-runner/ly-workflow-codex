...
- 移除 `migrateLegacyConfig`、`hasCoexistingLegacyLyProducts` 及其调用点、测试用例。
...
- `~/.codex/lyx/worktrees/`：这个子目录可能包含**实际的 git worktree**（工作目录 + 未提交改动）。裸 `rm -rf` 一个仍被主仓库 `.git/worktrees/` 元数据引用的 worktree 目录会留下孤儿引用（`git worktree list` 报错、后续 `git worktree add` 用同名分支可能冲突）。因此：
  - 遍历 `worktrees/` 下的子目录，用 `git rev-parse --git-dir`（在该子目录下执行）判断是否仍是一个有效的 git worktree。
  - 存在有效 worktree → 警告并跳过删除该子目录，提示用户先用 `lycx worktree remove <name>` 清理。
  - `worktrees/` 为空、不存在、或其下已无有效 worktree → 随 `config.toml`/`prompts/` 一并删除整个 `~/.codex/lyx/` 目录。
  - 判断失败（权限问题等）时保守起见按"存在 worktree"处理（跳过删除并警告），不冒险误删。
...
- **[风险] 大量文档/模板/spec 文本出现"路径漂移"（改了代码却漏改某处文档字符串）**→ **缓解**：tasks.md 按文件逐条列出所有已定位的 `~/.ly` 出现位置（含 3 份 delta spec 已覆盖的 spec 文本），实施阶段逐项核对；`grep -rn "~/.ly" src/ templates/ *.md openspec/specs/` 作为完成态验证手段写入 tasks.md 的验证步骤。
