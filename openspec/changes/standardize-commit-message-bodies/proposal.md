...
当前 `@lyx-commit` 对 message 正文只有弱建议，propose/apply/archive/review 修复等自动阶段提交又只定义了 subject + trailer，导致生成的提交普遍只有一句话，丢失动机、改动要点与影响范围。提交历史是审查、回溯和 changelog 的基础，需要把 `@lyx-commit` 的消息体规范提升为所有 lyx 显式生成 message 的提交的统一基线。

## What Changes

- 把 `@lyx-commit` 定义为 lyx 显式生成 message 的提交的基础规范：subject 使用 Conventional Commits 前缀，正文至少说明动机、改动与影响范围。
...
- 非 change 生命周期的 lyx 提交（init/release/changelog/publish、propose 切分支前的 WIP commit、worktree `--local` 的 `.gitignore` commit）也遵守基础正文规范，但不追加 `Change-Stage` / `Change-Name` trailer。
- git 原生生成的 merge / revert / cherry-pick 默认 message 不在本规范范围；只有模板显式提供 message 的提交才纳入。
- release/publish 的 `npm version` 自动提交路径需要避免默认短 commit；若生成版本提交，必须产出具正文的 message，或改为 `--no-git-tag-version` 后按规范手动提交再打 tag。
- 更新模板与 OpenSpec specs，使“所有 lyx 显式生成 message 的提交复用 `@lyx-commit` 正文规范”可检查、可审查。
...
- `commit-conventions`: 从 change 生命周期 trailer 约定扩展为所有 lyx 显式生成 message 的提交的基础正文规范，并定义自动阶段提交如何复用该规范。
...
- 模板：`templates/skills-codex/commit.md`、`propose.md`、`apply.md`、`archive.md`、`review-plan.md`、`review-code.md`、`init.md`、`release.md`、`changelog.md`、`publish.md`、`worktree.md`
