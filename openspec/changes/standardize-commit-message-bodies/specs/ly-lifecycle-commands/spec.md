...
`/ly:init`、`/ly:apply`、`/ly:archive` 与 `/ly:propose` 生成的自动提交 SHALL 按 `commit-conventions` 的“lyx 生成提交统一复用 `@lyx-commit` 正文规范”生成 message 正文。change 生命周期命令（apply/archive/propose）SHALL 在正文后追加既有 change trailer；非 change 生命周期命令（init）SHALL 遵守正文规范但不追加 `Change-Stage` / `Change-Name` trailer。
