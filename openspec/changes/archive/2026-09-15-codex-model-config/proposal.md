...
- `lycx init` 模型三连候选与 `lycx` 菜单的审查模型编辑候选改为 `[默认继承当前会话模型（留空）] + spawnableModels`，**不再以 provider `/models` 为候选来源、不再自由输入**；既有值非空但不在列表时，列表附加"保留当前值（不在可用列表，警告）"项，SHALL NOT 静默丢弃或覆盖（保留既有"不丢配置"语义）。注：menu 实际仅编辑 `reviewModel` 单字段，候选语义与 init 共用。
...
- 模板运行时（review-plan/review-code/apply）模型校验规则（判定语境限定在"模型校验"内）：配置的模型非空且 ∉ spawnableModels → **明确报告"子代理模型配置无效（<model> 不在可用列表，请运行 `lycx doctor` 或配置 `[codexHost] spawnableModels`）"并停止该关卡转人工改配，不回退当前会话直接执行**，不落入笼统的"运行期失败"终止；留空（未配置）→ 回退继承当前会话模型；**配置合法但宿主无 subagent 能力或初始 spawn 失败 → 仍按既有口径回退当前会话直接执行并如实报告**，与"配置无效"两分支分明、不得混淆。spawn 失败报错原文含 `Available models: ...` 时如实展示，供用户据实维护 `spawnableModels`。模板安装时注入**内置默认列表文本（仅作后备）**，生效清单一律由模板运行时读取 `~/.ly/config.toml` 决定；读取/解析失败 → 明确提示"无法读取配置，请运行 `lycx doctor` 检查"，SHALL NOT 按"未配置"静默继承回退。列表不写死在模板正文。
- **BREAKING（行为）**：模型三连移除"自定义输入"入口与 `/models` 拉取依赖，自由输入的旧用法被候选列表替换；存量非法配置值不自动纠正，仅警告+C 级暴露（doctor FAIL / init 警告项）。
- 文档（README/CLAUDE.md）与 i18n 文案同步；`glm-5.3-flash` 需 `reasoning_effort: low` 的参数坑写入 init 现状检测提示（背景说明，不新增模板参数通道，具体落地见 tasks 3.1/7.1）；`spawnableModels` 的维护方式在文档与 init 提示中明示（手改 `~/.ly/config.toml`，格式见 spec 示例；编辑交互入口列为后续增强）。
...
- `src/utils/installer-template.ts`：注入内置默认列表文本（仅作后备），签名扩展传配置
