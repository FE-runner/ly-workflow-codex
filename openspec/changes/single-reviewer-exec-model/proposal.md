...
- **配置字段语义降级**：`reviewModelB` / `reviewReasoningEffortB` 保留字段但标注为弃用路径（不再被审查流程读取使用）；init 向导"模型三连"改"模型二连"（不再采集 reviewModelB），`lycx doctor` 检查项同步调整（B 字段输出弃用提示）。
...
- `ly-lifecycle-commands`: apply/propose 编排描述中的 coding subagent fork 语义改为非 fork + context.md，全自动路径措辞同步。
...
- **规格**：`openspec/specs/ly-review-gates/`、`subagent-agent-config/`、`ly-propose-flow/`、`ly-lifecycle-commands/` 基线 requirement 大幅改写；新增 `review-context-artifact/`
- **源码**：`src/` 中 lycx doctor 检查项（子代理模型配置）、init 向导模型采集（"三连"改"二连"）、类型定义（`reviewModelB` 注释标注弃用）及对应测试
