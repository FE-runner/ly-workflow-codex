...
**Reason**: 审查执行模型改为"单审查 subagent（非 fork）+ 主会话裁决"：双 agent 并行独立审 + 交换结论共识的机制整体移除，token 成本与轮内延迟结构性偏高，且主会话裁决环节本已存在于审查-修复循环中，双审共识对裁决质量的边际贡献不抵其开销。

**Migration**: 由 ADDED Requirement「审查关卡以单审查 subagent（非 fork）执行」替代 spawn 语义（单 agent、非 fork、消费 context.md）；由 ADDED Requirement「Critical 裁决：认可即修复、不认可必须附可核验依据」承担原"共识归并/分歧时序"的质量把关职责（主会话逐条裁决 + 驳回硬线）；`reviewModelB`/`reviewReasoningEffortB` 字段降级为弃用（见 `subagent-agent-config` 能力）。

### Requirement: Critical 需先经当前会话判断是否认可, 不认可则触发分歧未决

**Reason**: "分歧未决"这一终止条件名实源于双审查 agent 的意见分歧；单审执行模型下不存在 agent 间分歧，该机制更名为"驳回硬线"并拆分为逐条口径与整轮口径（见「审查-修复循环与终止条件」的终止条件 5），同时补入"不认可必须附可核验依据"的裁决质量约束。

**Migration**: 由 ADDED Requirement「Critical 裁决：认可即修复、不认可必须附可核验依据」替代本条的裁决职责；终止触发（原"分歧未决"）由「审查-修复循环与终止条件（review-code / review-plan 共用）」的终止条件 5"驳回硬线"承接。

### Requirement: 审查调用失败视为独立终止条件

**Reason**: 单审执行模型下失败分类简化：删除"单一 agent 失败且另一 agent 结论完整"降级条款与"交换结论长链失败兜底"（两者均为双审/交换结论机制的专属路径，机制已移除），失败分类收敛为运行期失败、环境级不可用、配置状态未知三类。

**Migration**: 由 ADDED Requirement「审查调用失败分类处理」替代——运行期失败/环境级回退/配置状态未知三类口径与原判定不变，删除的仅为双审专属降级与兜底路径；失败 SHALL NOT 归入"驳回硬线"（调用失败非裁决分歧）。
...
- **THEN** 主会话重新 spawn 一个全新审查 subagent（非 fork, 只携带 TASK），TASK 增量携带第一轮全部 Critical 逐字原文与原路径清单，不假定第一轮 subagent 仍存活
