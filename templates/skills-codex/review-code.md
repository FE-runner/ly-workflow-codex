...
- **运行期失败**（spawn 后等待超时/卡死、返回内容格式不符或不含有效结论（含空响应与疑似截断，见"审查返回有效性判定"）、双审查任一 agent 调用失败且无法按回退口径继续、回退不可行或回退后仍失败）→ 视为**独立终止条件**，如实报告原因（退出码/超时说明/原始输出片段，含已取得的部分结论，如有）并停止循环，**不得**把失败或超时等同于"本轮无 Critical"或视为清零通过。
