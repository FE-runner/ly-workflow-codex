...
| 目录 | 用途 | 安装目标 |
|------|------|----------|
| `skills-codex/` | 14 个 SKILL.md 模板（codex 宿主单 Agent 版，`name`/`description`/`argument-hint` frontmatter；审查走**双审查 subagent**、实施走 **coding subagent**——fork 当前会话上下文 + 范围点名，模型与推理档经模板指示 + 宿主能力落实，无 wrapper/OVERALL/条件块） | `~/.agents/skills/lyx-*/SKILL.md` |
...
| `review-plan.md` | 真逻辑 | 双审查 subagent 审方案（`plan-reviewer.md`，fork 上下文 + 范围点名；模型 `reviewModel`/`reviewModelB`，非空推理档 `reviewReasoningEffort`/`reviewReasoningEffortB`），独立审→交换→共识，分歧主会话拍板；审查-修复循环（全局轮数上限 5，清零优先），清零后统一提交 |
| `review-code.md` | 真逻辑 | 双审查 subagent 审代码（`reviewer.md`，fork 上下文 + 范围点名；模型与推理档配对同 review-plan），Critical/Warning/Info 分级，同循环 |
...
| 占位符 | 说明 |
|--------|------|
| `{{REVIEW_MODEL}}` | **历史兼容占位**（subagent 多 Agent 模式已不再使用）：模型与推理档经模板指示 + 宿主 spawn 能力落实（`reviewModel`/`reviewModelB`/`codingModel` 与 `reviewReasoningEffort`/`reviewReasoningEffortB`/`codingReasoningEffort` 由模板运行时读取，模型未配置回退当前会话模型，推理档空白不传），模板不含模型或推理档渲染占位符；`{{REVIEW_MODEL}}` 处理仅保留给历史模板/旧安装位渲染兼容 |
