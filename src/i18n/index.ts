...
const zhCN = {
  ...
  init: {
    ...
    host: {
      reviewModelPrompt: '审查 agent A 模型（默认继承当前会话模型（留空）+ [codexHost] spawnableModels 生效清单；agent B / coding 模型请运行 lycx init 配置）',
      reviewModelHint: '候选 = 默认继承当前会话模型（留空）+ spawnableModels 生效清单（配置值或内置默认）；不在清单内的既有值以"保留当前值（警告）"项保全，不静默丢弃',
    ...
    codexStatus: {
      ...
      spawnableHint: '可 spawn 清单维护：手改 ~/.ly/config.toml 的 [codexHost] spawnableModels = ["model-a", "model-b"]（未配置回退内置默认；编辑交互入口为后续增强）。',
    },
    ...
    model: {
      ...
      trioCandidatesHint: '候选 = 默认继承当前会话模型（留空）+ [codexHost] spawnableModels 生效清单（配置值或内置默认）',
...
const en: typeof zhCN = {
  ...
  init: {
    ...
    host: {
      reviewModelPrompt: 'Review agent A model (default: inherit current session model (empty) + effective [codexHost] spawnableModels list; agent B / coding models can be configured via lycx init)',
      reviewModelHint: 'Candidates = inherit current session model (empty) + effective spawnableModels list (configured or built-in default); out-of-list existing values are preserved via a "keep current value (warning)" entry, never silently dropped',
    ...
    codexStatus: {
      ...
      spawnableHint: 'Maintaining the spawnable list: hand-edit [codexHost] spawnableModels = ["model-a", "model-b"] in ~/.ly/config.toml (falls back to the built-in default when unset; an editing entry point is future work).',
    },
    ...
    model: {
      ...
      trioCandidatesHint: 'Candidates = inherit current session model (empty) + effective [codexHost] spawnableModels list (configured or built-in default)',
