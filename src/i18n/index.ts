...
const zhCN = {
  ...
  init: {
    ...
    host: {
      reviewModelPrompt: '审查 agent A 模型（默认继承当前会话模型（留空）+ 生效清单：spawnableModels（或内置默认）∪ 已配置模型字段值 ∪ codex 主模型；agent B / coding 模型请运行 lycx init 配置）',
      reviewModelHint: '候选 = 默认继承当前会话模型（留空）+ 生效清单（spawnableModels 配置值或内置默认；未显式配置时并入已配置模型字段值与 codex 主模型，用户已配置的模型按配置列出）；不在清单内的既有值以"保留当前值（警告）"项保全，不静默丢弃',
    ...
    codexStatus: {
      ...
      notDetected: '未检测到',
      reasoningHint: '提示：部分第三方模型（如 glm-5.3-flash）默认推理参数不可用，需显式配置 reasoning_effort: low 才能 spawn。该提示仅为背景说明，本向导不新增推理参数配置通道，相关报错如实展示即可。',
      spawnableHint: '可 spawn 清单维护：手改 ~/.ly/config.toml 的 [codexHost] spawnableModels = ["model-a", "model-b"]（未配置回退内置默认，并自动并入已配置模型字段值与 codex 主模型；编辑交互入口为后续增强）。',
    },
    ...
    model: {
      ...
      trioCandidatesHint: '候选 = 默认继承当前会话模型（留空）+ 生效清单（spawnableModels 配置值或内置默认；未显式配置时并入已配置模型字段值与 codex 主模型）',
      ...
      customPrompt: '输入模型名（自定义，可不在生效清单内；留空可取消）',
      keepCurrentChoice: '保留当前值 {{model}}（不在可用列表，警告）',
    },
  },
  doctor: {
    modelConfig: {
      label: 'Codex 子代理模型配置',
      okUnset: '{{key}} 未配置（回退当前会话模型；以主会话模型本身可 spawn 为前提）',
      okInList: '{{key}}={{model}}（在可用清单内）',
      failNotInList: '{{key}}={{model}} 不在可用清单内（请配置 [codexHost] spawnableModels 或改用列表内模型）',
      listBuiltin: '生效清单（内置默认，并入已配置模型/主模型）：{{list}}',
...
const en: typeof zhCN = {
  ...
  init: {
    ...
    host: {
      reviewModelPrompt: 'Review agent A model (default: inherit current session model (empty) + effective list: spawnableModels (or built-in) ∪ configured model fields ∪ codex main model; agent B / coding models can be configured via lycx init)',
      reviewModelHint: 'Candidates = inherit current session model (empty) + effective list (configured spawnableModels or built-in default; when spawnableModels is unset, configured model fields and the codex main model are merged in — configured models are listed as configured); out-of-list existing values are preserved via a "keep current value (warning)" entry, never silently dropped',
    ...
    codexStatus: {
      ...
      reasoningHint: 'Note: some third-party models (e.g. glm-5.3-flash) cannot spawn with the default reasoning parameters and need an explicit reasoning_effort: low. This is background info only — the wizard adds no reasoning-parameter channel; related errors are shown as-is.',
      spawnableHint: 'Maintaining the spawnable list: hand-edit [codexHost] spawnableModels = ["model-a", "model-b"] in ~/.ly/config.toml (falls back to the built-in default when unset, merging in configured model fields and the codex main model; an editing entry point is future work).',
    },
    ...
    model: {
      ...
      trioCandidatesHint: 'Candidates = inherit current session model (empty) + effective list (configured spawnableModels or built-in default; when spawnableModels is unset, configured model fields and the codex main model are merged in)',
      ...
      customPrompt: 'Enter a model name (custom, may be outside the effective list; empty to cancel)',
      keepCurrentChoice: 'Keep current value {{model}} (not in available list, warning)',
    },
  },
  doctor: {
    modelConfig: {
      label: 'Codex subagent model config',
      okUnset: '{{key}} not configured (falls back to the current session model; assuming the main session model itself can be spawned)',
      okInList: '{{key}}={{model}} (in the available list)',
      failNotInList: '{{key}}={{model}} is NOT in the available list (configure [codexHost] spawnableModels or switch to an in-list model)',
      listBuiltin: 'effective list (built-in default, with configured models / main model merged in): {{list}}',
