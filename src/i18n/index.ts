...
const zhCN = {
  ...
  init: {
    ...
    codexStatus: {
      ...
      spawnableHint: 'agent 模型需额外配置（不做清单强校验）：能否 spawn 由环境实际能力决定。验证示例：对话中让 Codex 用目标模型 spawn 一个子代理执行简单任务（如回复 ok），报错含 Unknown model ... Available models: ... 即当前不支持；[codexHost] spawnableModels 仅作提示参考，可手改 ~/.ly/config.toml',
    },
